...
class Database
{
    ...
    function input_buku($judul, $id_penulis, $id_kategori, $jumlah_hlm, $tahun_terbit, $tgl_masuk, $sinopsis, $nama_file, $cover)
    {
        ...
        if ($tipe_file == "application/pdf" && $tipe_file_cover == "image/jpeg" || $tipe_file_cover == "image/png") //mengecek apakah file tersebut pdf atau bukan
        {
            ...
            if ($sql) { // Cek jika proses simpan ke database sukses atau tidak
                ...
                echo "<meta http-equiv='refresh' content='0;admin/list_buku.php'>";
            } else {
                // Jika Gagal, Lakukan :
            ...
            // header('location:admin/list_buku.php?pesan=upload-berhasil');
        } else {
    ...
    function input_penulis($penulis)
    {
        ...
        header('location:admin/list_penulis.php?pesan=tambah-penulis-berhasil');
    }
    ...
    function input_kategori($kategori)
    {
        ...
        mysqli_query($this->con, $sql);
        header('location:admin/list_kategori.php?pesan=tambah-kategori-berhasil');
    }
    ...
    {
        $sql = mysqli_query($this->con, "DELETE FROM penulis WHERE id_penulis='$id_penulis'");
        if ($sql) { // Cek jika proses simpan ke database sukses atau tidak
            // Jika Sukses, Lakukan :
            echo "<script>alert('Data Berhasil Dihapus')</script>";
            echo "<meta http-equiv='refresh' content='0;admin/list_penulis.php'>";
        } else {
            // Jika Gagal, Lakukan :
            echo "<script>alert('Maaf, Terjadi kesalahan saat mencoba untuk menghapus data ke database.')</script>";        //Pesan jika proses tambah gagal
            echo "<meta http-equiv='refresh' content='0;admin/list_penulis.php'>";
    ...
    {
        $sql = mysqli_query($this->con, "DELETE FROM kategori WHERE id_kategori='$id_kategori'");
        if ($sql) { // Cek jika proses simpan ke database sukses atau tidak
            // Jika Sukses, Lakukan :
            echo "<script>alert('Data Berhasil Dihapus')</script>";
            echo "<meta http-equiv='refresh' content='0;admin/list_kategori.php'>";
        } else {
            // Jika Gagal, Lakukan :
            echo "<script>alert('Maaf, Terjadi kesalahan saat mencoba untuk menghapus data ke database.')</script>";        //Pesan jika proses tambah gagal
            echo "<meta http-equiv='refresh' content='0;admin/list_kategori.php'>";
    ...
    function update_buku($id_buku, $judul, $id_penulis, $id_kategori, $jumlah_hlm, $tahun_terbit, $sinopsis, $cover)
    {
        ...
        if ($tipe_file_cover == "image/jpeg" || $tipe_file_cover == "image/png") //mengecek apakah file tersebut pdf atau bukan
        {
            ...
            header('location:admin/list_buku.php?pesan=update-buku-berhasil');
    ...
    {
        $sql = mysqli_query($this->con, "UPDATE penulis SET id_penulis='$id_penulis', penulis='$penulis' WHERE id_penulis='$id_penulis'");
        mysqli_query($this->con, $sql);
        header('location:admin/list_penulis.php?pesan=update-penulis-berhasil');
    }

    function update_kategori($id_kategori, $kategori)
    {
        $sql = mysqli_query($this->con, "UPDATE kategori SET id_kategori='$id_kategori', kategori='$kategori' WHERE id_kategori='$id_kategori'");
        mysqli_query($this->con, $sql);
        header('location:admin/list_kategori.php?pesan=update-kategori-berhasil');
    }
}
